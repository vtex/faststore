...
```bash
export CA_TOKEN=$(aws codeartifact get-authorization-token --domain main --domain-owner 053131491888 --region us-east-1 --query authorizationToken --output text)
```

> **Tip:** Verify that the token is set by printing it with `echo $CA_TOKEN`. It should display your auth token. If nothing is printed, the previous step didn't work — run the raw AWS command below to check for errors:
> ```bash
