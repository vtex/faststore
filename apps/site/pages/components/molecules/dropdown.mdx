...
import { CaretDown, ArrowElbowDownRight } from '@faststore/components'
...
<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection>
      <Dropdown>
        <DropdownButton icon={<CaretDown />}>
          {'Dropdown'}
        </DropdownButton>
        <DropdownMenu>
          <DropdownItem icon={<ArrowElbowDownRight />}>
            {'Dropdown Item 1'}
          </DropdownItem>
          <DropdownItem icon={<ArrowElbowDownRight />}>
            {'Dropdown Item 2'}
...
<TokenTable>
  ...
  <TokenDivider />
  <TokenRow
    token="--fs-dropdown-item-icon-min-width"
    value="1.125rem"
  />
  <TokenRow
