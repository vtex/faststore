...
export const getStaticProps = () => {
  ...
  const components = ['Navbar.tsx', 'NavbarHeader.tsx', 'NavbarRow.tsx', 'NavbarButtons.tsx']
  const [navbarProps, navbarHeaderProps, navbarRowProps, navbarButtonsProps ] = getComponentPropsFrom(navbarPath, components)
  return {
    props: {
...
export const NavbarPropsSection = ({ component }) => {
  ...
  const { navbarProps, navbarHeaderProps, navbarRowProps, navbarButtonsProps } = useSSG()
  return {
...
Represents the page header and groups some features like: `Search`, `Regionalization`, `SignIn`, `CartSidebar`. This component is part of the [Navigation](/components/features/navigation) feature.
