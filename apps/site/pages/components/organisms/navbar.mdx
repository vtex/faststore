...
The [NavbarLinks](/components/molecules/navbar-links) and [NavbarSlider](/components/molecules/navbar-slider) components are used as child components.

<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection containerStyle={{ display: 'block' }}>WIP</OverviewSection>
  </Tab>
  <Tab>```tsx WIP ```</Tab>
</Tabs>
