...
The `CartSidebar` displays the summary of items added to the cart along with detailed information, and checkout option. The final CartSidebar component is a compound of the following:

</header>

- `CartSidebarList`: wraps a series of `CartItem`s and `Gifts`.
- `CartSidebarFooter`: wraps an `OrdeSummary` and probably the Checkout Button.
