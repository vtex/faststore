...
1. Create a new `WebFonts.tsx` file in `src/fonts` folder, and add a `<link>` tag for your `font-family` of choice.
...
2. In your custom theme file, usually `src/themes/custom-theme.scss`, use the `--fs-text-face-body` token with the `font-family` you chose.
...
3. Run `yarn dev` and check the browser to see the `font-family`.
