...
2. In the `fragments` folder, create the file named `ServerProductPage.ts`. 
  ...
  ```bash copy
  touch src/fragments/ServerProductPage.ts
  ```
...
`
```
Now, the `customData` is available to be consumed by following the [Consuming FastStore API extension with custom components](/docs/api-extensions/consuming-api-extensions)
