...
<Callout type="warning" emoji="⚠️">
  **By overriding a component, you lose all the integration developed on
  [@faststore/core](/docs/getting-started/3-project-structure#faststorecore) for
  that component.** We highly recommend trying first [overriding
  props](/docs/building-sections/overriding-components-props) and/or using our
  [theming tools](/docs/themes/overview) to achieve the desired behavior without
...
1. Choose the native section to be customized from the [list of available native sections](/docs/reference-guides/list-of-native-sections).
...
1. Choose a component to override from the [list of overridable components for each native section](/docs/reference-guides/list-of-native-sections). In this example, we are overriding the `Price` component.
