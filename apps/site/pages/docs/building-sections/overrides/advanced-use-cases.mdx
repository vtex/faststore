...
This guide demonstrates an advanced use case for `getOverriddenSection` in FastStore, focusing on customizing the icon of an "Alert" section to display an image instead.

## Context

- You want to display an image alongside the text in your alert messages.
- The pre-built "Alert" section provides an "icon" prop for customization, but you want more control and flexibility over the displayed content.
...
2. **Define the `AlertWithImageProps` interface:**
   ...
   - It inherits all props from the original "Alert" component, excluding the "icon" prop as we intend to replace it with an image.
