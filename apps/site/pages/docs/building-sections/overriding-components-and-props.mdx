...
<Callout type="warning" emoji="⚠️">
  When overriding a component, it's important to note that you will lose the
  integration developed in
  [`@faststore/core`](/docs/getting-started/3-project-structure#faststorecore)
  for that component. To avoid compromising the existing integration, we highly
  recommend to consider [overriding
  props](#1-override-native-component-by-overriding-its-props) or utilizing the
  [theming tools](/docs/customizing/components) provided by `@faststore/core` to
  achieve the desired behavior. For more information about the available props
  for each component, refer to the [Components UI](/components) guide.
</Callout>
...
<Tab>```bash mkdir src\components src\components\overrides ```</Tab>
...
<Tab>```bash touch src\components\overrides\ProductDetails.tsx ```</Tab>
...
```tsx filename="ProductDetails.tsx"
import { SectionOverride } from '@faststore/core/src/typings/overrides'
...
<Tab>```bash touch src\components\CustomBuyButton.tsx ```</Tab>

</Tabs>

<Callout type="info" emoji="ℹ️">
  When creating a custom component, it's important to choose a name that
  distinguishes it from the native component. For example, you can use a name
  like `CustomComponentName`.
</Callout>
