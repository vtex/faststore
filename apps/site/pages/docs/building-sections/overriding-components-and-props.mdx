...
All component customizations in FastStore are performed within a section and to override props, you need to choose the desired native section and component from the [List of available native sections](/docs/reference-guides/list-of-native-sections#faststore-native-sections).
...
1. After choosing a native section to be customized from the [list of available native sections](/docs/reference-guides/list-of-available-native-sections), open your FastStore project in any code editor of your preference.
2. Go to the `src` folder, create the `components` folder and inside it, create the `overrides` folder.
...
1. Open the `ProductDetails.tsx` file created in the [previoulsy step](#step-1-setting-up-the-overrides-folder) and add the following code:
...
1. Inside the `components` folder, create a file named `CustomBuyButton.tsx`. *If you haven't created the `components` folder yet, see the [Setting up the overrides folder](/docs/building-sections/overriding-components-and-props#step-1---setting-up-the-overrides-folder) step.*
