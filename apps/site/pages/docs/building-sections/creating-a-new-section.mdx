...
In this guide, you'll learn how to create a new section for your store and make it available in the VTEX Headless CMS. This solution is useful when your store requires a section not natively available in FastStore.
...
4. In the `sections.json` file add the new section that you want to display in the Headless. The schema below defines how the Headless CMS renders a section:

```json filename="sections.json"
[
  {
    "name": "CallToAction",
    "schema": {
...
This new section receives a title and a link pointing to somewhere in the store.

### Step 2: Creating the new section

To render the `CallToAction` section you created in the previous step, you need to create a this section component.
...
1. In the terminal, run `faststore cms-sync`. This command will synchronize the new section you created with the @faststore/core.
