...
Intending to separate what comes from VTEX and what comes from third-party APIs, as well as improve the metrics collection, is recommended to use the following predefined folder structure in the store's repository for the new types and their respective resolvers:
