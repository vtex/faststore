...
There are cases where you need to render a specific template for products inside the `department` without considering the `category` or even a `subcategory`. For this case, you can create a `department` template [/office](https://starter.vtex.app/office/chairs).
