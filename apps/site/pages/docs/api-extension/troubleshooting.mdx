...
If you notice differances between the GraphQL schema in your deploy preview or production environment compared to your development setup, it may be due to the schema not being optimized since the development server's initiation. 
Typically, the build process optimizes the schema before deployment to ensure it accurately reflects the schema declared in the store's code.

`Question: Can we recommend then to run `yarn generate`?`
...
To access more detailed error information, use the `--debug` flag when manually running the `yarn generate` command to see detailed errors on why the generation has failed. For example: `{add a screenshot example}`
...
If your changes are not visible in production, this means you must have not commited them to the branch you're currently working on. If you see a different GraphQL Schema, queries or data during development, refer to the [GraphQL changes not visible during development](/docs/api-extension/troubleshooting#graphql-changes-not-visible-during-development) topic.