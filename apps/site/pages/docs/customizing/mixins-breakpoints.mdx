...
We use the [Sass `@mixin`](https://sass-lang.com/documentation/at-rules/mixin/) to allow you to define styles that can be re-used throughout your application. We divide them into 3 categories: `Global Mixins`, `Layout & Spacing Mixins` and `Focus Ring Mixins`.
...
The process of overriding those mixins is similar to the customizing breakpoints steps. If you already have a `custom-mixin.scss` file, you can skip to Step 3.
...
4. Make the changes on the `@mixin` declaration.
...
5. Run `yarn dev` in the terminal to start the development server. You should see that all components that use the `focus-ring` are now with a red shadow. Click the input to see the change.
