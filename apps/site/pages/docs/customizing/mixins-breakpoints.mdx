...
5. Run `yarn dev` to start the development server. You should see that all components that use the `focus-ring` now have a red shadow when focused. Click on the input to see the change.
