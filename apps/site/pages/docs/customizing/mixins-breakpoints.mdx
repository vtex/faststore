...
<Callout type="warning" emoji="⚠️">
  Modifying these breakpoints implies changes to certain global tokens. See
  [Grid & Layout](/docs/global-tokens/grid-and-layout#breakpoints) for more
  details.
</Callout>
...
<Tabs items={['macOS and Linux', 'Windows']}>
  <Tab>```bash copy mkdir src/styles ```</Tab>
  <Tab>```bash copy mkdir src\styles ```</Tab>
</Tabs>
...
<Tabs items={['macOS and Linux', 'Windows']}>
  <Tab>```bash copy touch src/styles/custom-mixins.scss ```</Tab>
  <Tab>```bash copy type nul > src\styles\custom-mixins.scss ```</Tab>
</Tabs>
...
<Callout type="warning" emoji="⚠️">
  You need to declare all the variants (phone, phonemid, tablet, notebook and
  desktop) even when changing only one of the values.
</Callout>
...
Overriding the predefined mixins follows a process similar to the [customizing breakpoints steps](/docs/customizing/mixins-breakpoints#customizing-breakpoints). If you already have a `custom-mixin.scss` file, you can skip to _Step 3_. Otherwise, follow the steps below:
