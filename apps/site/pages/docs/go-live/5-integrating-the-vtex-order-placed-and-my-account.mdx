...
Notice that, since we're using a subdomain for these integrations that is unrelated to FastStore itself, we'll use [VTEX IO](https://developers.vtex.com/docs/guides/vtex-io-documentation-what-is-vtex-io) in this guide, a VTEX solution for creating VTEX apps. However, there is no need to worry about diving into VTEX IO concepts in order to complete this guide.
