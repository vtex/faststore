...
## Prerequisites
Before clonning the store's repository ensure that you have the following
...
<Callout type="info" emoji="ℹ️">
  Although you might opt to use an alternative code editor, note that using Visual Studio Code might help you follow along with the FastStore documentation since our documentation may include screenshots from VS Code.
</Callout>

## Step 1: Clonning the Store's Repository
1. Clone your store's repository that was created during the [Onboarding](/docs/getting-started/onboarding#repository-and-live-store-created) by running the following command in the terminal:
```bash
...
2. Change into the working directory (e.g., cd `faststore.store`) and install all the dependencies listed within the `package.json` file:
...
```
<Callout type="info" emoji="ℹ️">
  The yarn dev command is a part of the FastStore CLI, a pre-configured binary in your starter project. This command enables you to execute code to keep your store's project up-to-date with the core package using the command-line interface. For more information, you can refer to the topic on [FastStore CLI](/tbd).
</Callout>
...
Let's make the first level of customization which is update the store's theme with [Theming](/docs/brandless). For this customization we are going to use the [Midnight theme](/tbd) and we will make some updates on it.

1. Open the `faststore.config.js` file and in `theme`, change the name to `soft-blue`.
...
Now, let's override the theme by changing the first color palette (`--fs-color-main-0: #ecf0ff`).

6. Change the color code to `#8db6fa`:

```scss
  ...
   // PALETTE
  --fs-color-main-0: ##8db6fa;
  ...
```

7. Save your changes and check your browser to see the changes you have made. You may need to refresh the page.
