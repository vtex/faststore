...
Now, let's customize a component. We will use the `Listing` variant from the [Price Component](/components/atoms/price) in this tutorial.

6. In the `soft-blue.scss` file, add the `data-fs-price-variant` data attribute with the `Listing` value: 
