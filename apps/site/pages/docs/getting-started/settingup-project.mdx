...
Now that your store is up and running, you're ready to set up your development environment and create a unique storefront that reflects your brand. Let's get started!
...
2. Clone your store's repository that was created during the [Onboarding](/docs/getting-started/onboarding#repository-and-live-store-created) by running the following command in the terminal:
```bash
    git clone https://github.com/vtex-sites/[store-name]
```
<Callout type="warning" emoji="⚠️">
Replace `[store-name]` for your store's repository.
</Callout>

3. Change into the working directory (e.g., cd `faststore.store`) by running the following command: 
...
6. In the `soft-blue.scss` file, add the `data-fs-price` and  `data-fs-price-variant` data attributes with the `Listing` value: 
...
7. Now, change the color of the `Listing` variant using its [local token](/components/atoms/price#design-tokens). For this tutorial, we will use the shade of red `#cb4242`:
```scss
  }
...
8. Save your changes and check your browser to see the new color of your listing price. You may need to refresh the page.
