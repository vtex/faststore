...
```
<Callout type="info" emoji="ℹ️">
  The yarn dev command is a part of the FastStore CLI, a pre-configured binary in your starter project. This command enables you to execute code to keep your store's project up-to-date with the [core package](/tbd) using the command-line interface. For more information, you can refer to the topic on [FastStore CLI](/tbd).
</Callout>
...
Now, let's customize a component. For this tutorial we will use the `Listing` variant from the [Price Component](https://evergreen.faststore.dev/components/atoms/price).

6. In the `soft-blue.scss` file, below `[data-fs-logo]`, add the `Listing` variant:
...
7. Save your changes and check your browser to see the new color of your listing price. You may need to refresh the page.
