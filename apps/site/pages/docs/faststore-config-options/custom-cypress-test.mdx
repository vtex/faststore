...
- Ensure that you are using `@faststore/core` version `2.1.56` or above and `@faststore/cli` version `2.2.6` or above. If you have a version that is lower than this requirement, change the following:
  ...
  2. Navigate to the `@faststore/core` and update it to be at least `2.1.56`.
  3. Navigate to the `@faststore/cli` and update it to be at least `2.2.6`.
  4. Run `yarn dev` to update your changes.
...
Just like any other project using Cypress, all standard Cypress folders and configuration files are supported. (see official Cypress configuration guide [here](https://docs.cypress.io/guides/references/configuration))
