...
Use this mutation for all shopping cart operations. It checks for changes between the cart presented in the UI and the cart stored in the ecommerce platform. If changes are detected, it updates the cart stored on the platform and then returns it. Otherwise, it returns `null`.
...
<PropsComponent
      name="cart"
      type="<code>IStoreCart</code>!"
      description="Shopping cart that should be validated, which is <code>IStoreOrder</code>, containing <code>orderNumber</code> and <code>acceptedOffer</code>."
      required="true"
    />
...
<PropsComponent
      name="acceptedOffer"
      type="[<code>IStoreOffer</code>!]"
      description="Array of offers (<code>IStoreOffer</code>) that are in the front-end cart."
      required="true"
    />
