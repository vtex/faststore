...
- [gatsby.store](https://github.com/vtex-sites/gatsby.store) based projects (using Gatsby v4): `src/server/index.ts`.

If the indicated file is in your project you will be able to run the local API. If you do not, copy the file from the latest version in the corresponding repository.
