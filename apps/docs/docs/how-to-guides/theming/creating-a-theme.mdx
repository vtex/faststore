...
4. Check the reference page of the component you wish to style to change any local token. For example:
   ...
   ```
6. Save your changes.
7. Open the `src/pages/_app.tsx` file and import the `my-theme.scss` stylesheet.

   ```jsx
   import '../styles/themes/my-theme.scss'
   ```

8. Go to `store.config.js` and change the `theme` to your file's name (e.g., `my-theme`):
