            ...
            <tr key={i} {...row.getRowProps()}>
              {row.cells.map((cell:any) => {
                if (cell.column.id == 'example') {
                  return (
                    <td key={i}>
                      <div style={JSON.parse(cell.value)} className="rounded-full w-8 h-8 border"/>
                    </td>
                  )
                } else if (cell.column.id !== 'type') {
                  return (
                    <td key={i} {...cell.getCellProps()}>
                      <CopyButton code={(cell.value)} className={`flex items-center p-2 rounded hover:text-rebelPink float-left`}/>
