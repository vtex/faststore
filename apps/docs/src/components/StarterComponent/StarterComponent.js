...
const StarterComponent = ({ starter }) => {
  return (
    <div className="w-80 my-5 hover:shadow-md">
      <div>
      ...
      <div className="p-4">
        <Link
          className="hover:no-underline text-rebelPink text-lg"
          href={starter.url}
        >
          {starter.name}
        </Link>

        <p className="text-details">{starter.details}</p>

        <div className="pt-5">
          <Link
            className="inline-block tracking-wider text-sm text-fontSecondary font-medium"
            href={starter.url}
          >
            SEE MORE →
          </Link>
