...
- `AccordionButton`: the button that is used to interact with its related `AccordionItem`, opening or closing the `AccordionPanel`.
- `AccordionPanel`: the panel that contains the contents of the `AccordionItem`.

## Props

All accordion-related components support all attributes also supported by the `<div>` tag, with the exception of the `AccordionButton` which renders the `<button>` tag.
