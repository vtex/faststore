...
export const VtexCommerce = (
  ...
  return {
    ...
    session: (search: string): Promise<Session> => {
      ...
      params.set(
        'items',
        'profile.id,profile.email,profile.firstName,profile.lastName,store.channel,store.countryCode,store.cultureInfo,store.currencyCode,store.currencySymbol,authentication.customerId,authentication.storeUserId,authentication.storeUserEmail,authentication.unitId,authentication.unitName,checkout.regionId,public.postalCode,public.geoCoordinate,public.country'
      )
