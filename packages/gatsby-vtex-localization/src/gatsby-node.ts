import { CreatePageArgs, CreateWebpackConfigArgs, Node, SourceNodesArgs } from 'gatsby'
...
const defaultMessagesPath = './i18n/messages'
...
export const sourceNodes = async (
  ...
  const { messagesPath = defaultMessagesPath, locales = localesArrayDefault, defaultLocale = 'en' } = options
  ...
  for (const locale of locales) {
    ...
    try {
      languageJson = await readJSON(`${messagesPath}/${locale}.json`, { encoding: "utf-8" })
    } catch (e) {
      languageJson = null
    }
    ...
    await outputFile(`${messagesPath}/${locale}-temp.json`, JSON.stringify(withDefaultMessage))

    const resultAsString = await compile([`${messagesPath}/${locale}-temp.json`], { ast: true })
...
export const onCreatePage = async ({ page, actions, getNodesByType }: CreatePageArgs) => {
  ...
  const languageConfig = getNodesByType('LanguageDataConfig')[0] as LanguageConfigNode
  ...
  for (const languageData of languageNodes) {
    ...
    createPage({
      ...
      matchPath: page.matchPath ? localizedPath(defaultLocale, locale, page.matchPath) : page.matchPath,
      context: {
        ...page.context,
