...
import { getGraphQLConfig } from '@vtex/graphql-api'
// import {
//   FilterObjectFields,
//   introspectSchema,
//   PruneSchema,
...
export const createSchemaCustomization = async (
  ...
  const { schema } = getGraphQLConfig({
    tenant: options.tenant,
    environment: options.environment,
  })

  gatsbyApi.actions.createTypes(schema)
}
...
type GatsbyResolvers = Record<string, GatsbyResolverTypeObj>
type GatsbyResolverTypeObj = Record<string, { resolve: ResolverFunction }>

type VtexResolvers = Record<string, VtexResolverTypeObj>
type VtexResolverTypeObj = Record<string, ResolverFunction>

export const createResolvers = (
  { createResolvers: createGatsbyResolvers }: CreateResolversArgs,
  options: Options
) => {
  const { resolvers }: { resolvers: VtexResolvers } = getGraphQLConfig({
    tenant: options.tenant,
    environment: options.environment,
  })

  const gatsbyResolvers: GatsbyResolvers = {}

  for (const typeName in resolvers) {
    gatsbyResolvers[typeName] = {}
    for (const fieldName in resolvers[typeName]) {
      gatsbyResolvers[typeName][fieldName] = {
        resolve: resolvers[typeName][fieldName],
      }
    }
  }

  createGatsbyResolvers(gatsbyResolvers)
}
