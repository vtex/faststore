...
function ProductComparisonToolbar({
  ...
  const { isOpen, setIsOpen, selectedProducts, clearProducts } = useProductComparison()
      ...
      {selectedProducts.length >= 1  && !isOpen ? (
        <footer data-fs-product-comparison-toolbar>
          ...
          <div>
            ...
            <Button variant="primary" disabled={selectedProducts.length < 2} onClick={() => setIsOpen(true)}>
              {compareLabel}
            </Button>
          </div>
        </footer>
